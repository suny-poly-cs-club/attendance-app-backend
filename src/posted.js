...
const RegisterSchema = object({
  firstName: string([maxLength(80, 'first name exceeds max length')]),
  lastName: string([maxLength(80, 'last name exceeds max length')]),
  email: string([
...
export const postEndpoints = (app, options, done) => {
  app.post('/sign-up', async (request, reply) => {
    ...
    const token = request.ctx.authManager.createToken(user);
